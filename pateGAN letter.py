...
###############################################################################
# 1) DEFINE THE MODELS (Conditional Generator, Teacher, Student)
###############################################################################
class ConditionalGenerator(nn.Module):
    """
    Conditional Generator: (noise_dim + one-hot(num_classes)) -> hidden -> data_dim.
    Produces synthetic samples (size = data_dim) conditioned on the provided label.
    """
    def __init__(self, noise_dim, data_dim, num_classes, hidden_dim=128):
        super(ConditionalGenerator, self).__init__()
        self.noise_dim = noise_dim
        self.num_classes = num_classes
        # New input dim = noise_dim + num_classes (using one-hot representation)
    ...
    def forward(self, z, labels):
        ...
        # Concatenate noise and condition vector along dimension 1
...
###############################################################################
# 3) The Multi-Class PATE-GAN Training Loop (for the conditional GAN)
###############################################################################
def train_pate_gan_multiclass(generator,
    ...
    The Conditional Generator now takes an extra label input.
    During generator training we pass the desired target label as condition.
    """
    ...
    for epoch in range(num_epochs):
        for batch_idx, (real_data, _) in enumerate(real_data_loader):
            ...
            # For the conditional GAN, pick a random condition (from real classes)
            random_cond = torch.randint(low=0, high=k_classes, size=(batch_size,), device=device)
            # Generate fake data using the condition label
            fake_data = generator(z, random_cond).detach()
            ...
            # Choose a random target label for each sample (condition label)
            gen_target = torch.randint(low=0, high=k_classes, size=(batch_size,), device=device)
            z = torch.randn(batch_size, noise_dim, device=device)
            # Pass both the noise and target label to the generator
            gen_data = generator(z, gen_target)
...
def main():
    ...
    #    Assume there's a target column named 'letter' with MULTIPLE classes
    # ------------------------------------------------------
    df = pd.read_csv("letter.csv")  # <--- EDIT PATH AS NEEDED

    # Encode letters (e.g., A, B, C, etc.) numerically
    label_encoder = LabelEncoder()
    df["letter"] = label_encoder.fit_transform(df["letter"])
    K = df["letter"].nunique()  # number of distinct real classes

    X = df.drop(columns=["letter"]).values  # all features
    y = df["letter"].values                # encoded labels in [0..K-1]
    ...
    # ------------------------------------------------------
    # 5. Build Conditional Generator & Student
    #    - Teacher => K classes
    #    - Student => K+1 classes (the extra one is "fake")
    # ------------------------------------------------------
    noise_dim = 100
    generator = ConditionalGenerator(noise_dim=noise_dim, data_dim=X.shape[1], num_classes=K, hidden_dim=500)
    student = Student(input_dim=X.shape[1], hidden_dim=500, num_classes=K+1)

    # ------------------------------------------------------
    # 6. Train the Multi-Class PATE-GAN (Conditional)
    # ------------------------------------------------------
    print("Starting Multi-Class Conditional PATE-GAN training...")
    trained_generator, trained_student = train_pate_gan_multiclass(
        ...
        k_classes=K,
        noise_dim=noise_dim,
        num_epochs=1000,     # Increase as needed
        lap_scale=0.1,     # Laplace noise scale
        lr=1e-4,
        device=device
    )
    print("Conditional PATE-GAN training complete!")
    ...
    #    We'll write features + label in columns
    # ------------------------------------------------------
