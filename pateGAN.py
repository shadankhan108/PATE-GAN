...
def main():
    ...
    # ------------------------------------------------------
    df = pd.read_csv("data.csv")  # <--- EDIT PATH AS NEEDED

    # Example: if 'letter' can be A, B, C, etc., we encode them numerically
    ...
    # ------------------------------------------------------
    noise_dim = 10
    generator = Generator(noise_dim=noise_dim, data_dim=X.shape[1], hidden_dim=128)
    ...
    # We'll generate in mini-batches for efficiency
    batch_size = 128
    ...
    for b in range(n_batches):
        ...
        # We choose a random label in [0..K-1] for each synthetic sample
    ...
    # ------------------------------------------------------
    # Let's use a simple Logistic Regression for demonstration
    ...
    print("Testing on real data (labels from original CSV):")
    real_preds = clf_synth.predict(X)  # shape: (N,)
    acc = accuracy_score(y, real_preds)
    print(f"TSTR Accuracy on real data: {acc:.4f}")

    # Also, if you want to see some details:
    # e.g., from sklearn.metrics import classification_report
    # print(classification_report(y, real_preds))
